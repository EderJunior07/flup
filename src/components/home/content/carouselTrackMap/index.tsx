...
interface ICarouselTrackMap {
  city: string;
}

const CarouselTrackMap = ({ city }: ICarouselTrackMap) => {
  ...
  const trackSpot = async () => {
    ...
    const snapshots = await spotsCollectionRef
      .where('status', '==', 1)
      ...
      {hasSpots ? (
        ...
        <>
          <Text style={{margin: 16}}>
            Nos perdoe, mas ainda não encontramos nenhuma pista em {city}
          </Text>
        </>
      )}
    </>
